...
fn calc_root_hash(hashes: &mut [B256]) -> B256 {
    ...
    while n > 1 {
        for i in (0..n).step_by(2) {
            hashes[i / 2] = hash_pair(hashes[i], hashes[usize::min(i + 1, n - 1)]);
        }
        n = (n + 1) / 2
    }
...
fn get_proof(hashes: &mut [B256], mut idx: usize) -> Vec<B256> {
    ...
    while n > 1 {
        if idx % 2 == 0 {
            proof.push(hashes[usize::min(idx + 1, n - 1)]);
        } else {
            proof.push(hashes[idx - 1]);
        }
        idx /= 2;
        for i in (0..n).step_by(2) {
            hashes[i / 2] = hash_pair(hashes[i], hashes[usize::min(i + 1, n - 1)]);
...
fn verify(root: B256, proof: &[B256], hashes: &[B256], mut idx: usize) -> bool {
    let mut hash = hashes[idx];
    for p in proof {
        if idx % 2 == 0 {
            hash = hash_pair(hash, *p);
        } else {
            hash = hash_pair(*p, hash);
        }
        idx /= 2;
    }
    return hash == root;
